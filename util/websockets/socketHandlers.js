const setupRentalSocket = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on("rentalEmergency", (data) => {
      const { rentalId, renterId, carId, emergencyType, location, message } = data;
  ...
  const emitRentalCreated = (rentalData) => {
    ...
    if (rentalData.renterId) {
      io.to(`renter_${rentalData.renterId}`).emit("myRentalCreated", rentalData);
    }
  ...
  const emitRentalCompleted = (rentalData) => {
    ...
    // Notify specific rooms
    if (rentalData.renterId) {
      io.to(`renter_${rentalData.renterId}`).emit("myRentalCompleted", rentalData);
    }
  ...
  const emitRentalCancelled = (rentalData) => {
    ...
    // Notify specific rooms
    if (rentalData.renterId) {
      io.to(`renter_${rentalData.renterId}`).emit("myRentalCancelled", rentalData);
    }
